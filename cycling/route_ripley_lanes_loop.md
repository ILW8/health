# Route: Ripley Lanes Loop (SW London → Surrey → Thames)

**Prepared:** 2 September 2026 (corrected 4 September 2026: Cobham turns)
**Purpose:** Back-pocket Z2 long ride for the 60–70 km weeks of the EPIC49 plan (weeks 1–3) and for winter weekends. Not tied to a date.
**Start/finish:** Raynes Park
**Distance:** ~68 km with the Richmond Park lap; ~56 km without
**Climbing:** ~350 m (mostly the park lap; the Surrey section is gently rolling)
**Time at Z2 (~23 km/h):** ~3 h moving, ~3.5 h with a cafe stop
**Direction:** Clockwise as written. Reverse works but puts the busy Portsmouth Road section at the end when you're tired.

Distances are estimates from the map; plot it in komoot/RideWithGPS before the first ride and correct this file with the real numbers.

---

## Turn-by-turn

| km | Waypoint | Notes |
|---|---|---|
| 0 | Raynes Park | Coombe Lane West → Norbiton → Kingston town centre. Urban, keep it Z1 as a warm-up. |
| 4.5 | Kingston | Pick up the A307 Portsmouth Road heading south along the river. |
| 8 | Thames Ditton / Long Ditton | Stay on Portsmouth Road. |
| 11.5 | Esher | Continue on the A307 through Esher and across Esher Common. **Busiest section of the ride** (Esher → Cobham): fast traffic, but a wide road and a standard club-run road. Earlier start = quieter. |
| 17 | Cobham, north-west edge | The A307 (Fairmile / Portsmouth Road) ends at its junction with the A245. **Turn left onto Between Streets**, heading south-east into town. Between Streets becomes the High Street. |
| 18 | Cobham, east end of High Street | **Turn right into Church Street** (the short street to St Andrew's church), which runs straight into **Downside Bridge Road** heading south over Downside Bridge. Traffic drops away here. |
| 19.5 | Downside | Cross Downside Common, continue on **Plough Lane → Ockham Lane**, under the M25. Narrow lanes, occasional gravel, watch for horses. |
| 23.5 | Ockham (Black Swan) | Turn right onto **Ockham Road North (B2039)** into Ripley. |
| 27 | **Ripley** | Cafe stop: Pinnocks (High Street). If you're not stopping, at least eat here. |
| 27.5 | Ripley, north end of High Street | **Newark Lane** past Newark Priory ruins and over the Wey. Flat and quiet. |
| 30 | Pyrford Lock | The Anchor pub is on the lock (alternative stop). Continue on Pyrford Road. |
| 33 | West Byfleet | Through the village. |
| 35 | Byfleet | **Brooklands Road (B374)** over the M25 towards Weybridge. |
| 39 | Weybridge | Through the town, then towards Walton. |
| 42.5 | Walton-on-Thames | **A3050 Walton Road → Hurst Road** past the reservoirs. |
| 48 | Hampton Court Bridge | Cross the bridge, then **A308 Hampton Court Road** along the south edge of Bushy Park to Hampton Wick. Flat, fast; good stretch for holding a steady 25 km/h. |
| 51.5 | Kingston Bridge | Cross back into Kingston. |
| 53 | Richmond Park, Kingston Gate | One clockwise lap (~10.7 km, no lights). Sawyer's Hill and Broomfield Hill will push HR up; that's fine, just don't race them. |
| 64 | Kingston Gate → Kingston | Back down to town. |
| 68 | Raynes Park | Done. |

---

## Variants

- **Short (~56 km):** skip the Richmond Park lap; Kingston Bridge → home.
- **Long (~78 km):** at Weybridge continue west to Chertsey, then Shepperton → Walton and rejoin the route. Flat, adds ~10 km of riverside road.
- **Long, hillier (~80 km):** two Richmond Park laps instead of one.
- **Wet/dark:** the Downside–Ockham lanes are the bit to drop. Go Cobham → A245 Byfleet Road → Byfleet instead (busier but lit and surfaced).

## Bail-out points (SWR to Waterloo via Wimbledon/Raynes Park)

Esher, Cobham & Stoke d'Abernon, West Byfleet, Byfleet & New Haw, Weybridge, Walton-on-Thames, Hampton Court. You are never more than ~15 min from a station.

## Pacing and fuelling (from the plan)

- Z2 means HR 125–150. Warm-up through Kingston at Z1. If HR drifts over 150 on the flat, ease off.
- Eat something at ~45 min (around Esher), at Ripley, and at Hampton Court. Two bottles.
- Use the Hampton Court → Kingston stretch to practise a steady 24–25 km/h without HR creeping into Z3.

## Log

| Date | Distance | Moving time | Avg HR | Notes |
|---|---|---|---|---|
| | | | | |
